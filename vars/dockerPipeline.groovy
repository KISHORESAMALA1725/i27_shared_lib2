...
// Move all method definitions outside the pipeline block
def buildApp() {
    ...
    pipeline {
        ...
        stages {
            ...
            stage('***** Deploy to DEV-ENV *****') {
                ...
                steps {
                    script {
                        deployToDocker('dev', "${env.DEV_HOST_PORT}", "${env.CONT_PORT}")
                    }
                }
            }
            ...
            stage('***** Deploy to TEST-ENV *****') {
                ...
                steps {
                    script {
                        deployToDocker('test', "${env.TEST_HOST_PORT}", "${env.CONT_PORT}")
                    }
                }
            }
            ...
            stage('***** Deploy to STAGE-ENV *****') {
                ...
                steps {
                    script {
                        imageValidation()
                        deployToDocker('stage', "${env.STAGE_HOST_PORT}", "${env.CONT_PORT}")
                    }
                }
            }
